...
import com.bugbycode.dao.oauth.TokenDao;
...
public class TokenStoreService implements TokenStore{
	...
	public OAuth2Authentication readAuthentication(String token) {
		...
		try {
			OauthAccessToken accessToken = tokenDao.queryByTokenId(extractTokenKey(token));
			if(accessToken == null) {
	...
	public OAuth2AccessToken readAccessToken(String tokenValue) {
		...
		try {
			OauthAccessToken accessToken = tokenDao.queryByTokenId(extractTokenKey(tokenValue));
			if(accessToken != null) {
	...
	public void removeAccessToken(String tokenValue) {
		tokenDao.deleteByTokenId(extractTokenKey(tokenValue));
	}
	...
	public OAuth2RefreshToken readRefreshToken(String tokenValue) {
		...
		try {
			OauthRefreshToken refresh = tokenDao.queryRefreshTokenByTokenId(extractTokenKey(tokenValue));
			if(refresh == null) {
	...
	public OAuth2Authentication readAuthenticationForRefreshToken(String token) {
		OAuth2Authentication authentication = null;
		try {
			OauthRefreshToken refresh = tokenDao.queryRefreshTokenByTokenId(extractTokenKey(token));
			if(refresh == null) {
	...
	public void removeRefreshToken(String token) {
		tokenDao.deleteRefreshByTokenId(extractTokenKey(token));
	}
	...
	public void removeAccessTokenUsingRefreshToken(String refreshToken) {
		tokenDao.deleteByRefreshToken(extractTokenKey(refreshToken));
	}
	...
	public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
		...
		try {
			OauthAccessToken oauthToken = tokenDao.queryByAuthenticationId(key);
			if(oauthToken != null) {
	...
	public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
		...
		List<OauthAccessToken> list = tokenDao.queryByClientIdAndUserName(clientId, userName);
		if(!list.isEmpty()) {
			for(OauthAccessToken token : list) {
				try {
					...
					e.printStackTrace();
					logger.error(e.getLocalizedMessage());
					tokenDao.deleteByTokenId(token.getTokenId());
				}
			}
	...
	public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
		List<OAuth2AccessToken> accessTokens = new ArrayList<OAuth2AccessToken>();
		List<OauthAccessToken> list = tokenDao.queryByClientId(clientId);
		if(!list.isEmpty()) {
			for(OauthAccessToken token : list) {
				try {
					accessTokens.add(deserializeAccessToken(token.getToken()));
