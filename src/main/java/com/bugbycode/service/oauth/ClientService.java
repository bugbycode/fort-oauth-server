...
public class ClientService implements ClientDetailsService {
	...
	public ClientDetails loadClientByClientId(String clientId) throws ClientRegistrationException {
		try {
			OauthClientDetails jdbcClient = clientDetailsDao.queryByClientId(clientId);
			if(jdbcClient == null) {
			...
			if(StringUtil.isNotBlank(scope) && StringUtils.hasText(scope)) {
				client.setScope(StringUtils.commaDelimitedListToSet(scope));
			}
			String resourceIds = jdbcClient.getResourceId();
			if(StringUtil.isNotBlank(resourceIds) && StringUtils.hasText(resourceIds)) {
			...
			if(StringUtil.isNotBlank(authorizedGrantTypes) && StringUtils.hasText(authorizedGrantTypes)) {
			...
			if(StringUtil.isNotBlank(webServerRedirectUri) && StringUtils.hasText(webServerRedirectUri)) {
			...
			if(StringUtil.isNotBlank(authorities) && StringUtils.hasText(authorities)) {
			...
			if(StringUtil.isNotBlank(additionalInformation) && StringUtils.hasText(additionalInformation)) {
			...
			if(StringUtil.isNotBlank(autoApprove) && StringUtils.hasText(autoApprove)) {
